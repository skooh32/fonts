...
части меню
Команды скрипта:
	/dep - быстрое снятия денег с депозита(более 200кк). Использование: перед снятием ввести /dep (1 раз), нажимать в банке "Снять деньги с депозита"
	/cnum - быстрый звонок по номеру телефона игрока
	/cid - быстрый звонок по id игрока
	/fi, fm, fu - принять, замутить, кикнуть игрока из семьи
	/getbtc - собирает биткоины видеокарт с помощью "флешки". (Флешка должна лежать на 1 странице инвентаря) Пример: /getbtc 1 (сбор биткоинов с 1-го дома)
	/arz - открыть это окно
	/calc - калькулятор. Пример: /calc 2+2*2
	/rep - быстрый репорт
	/de, m4, sh, rf, ak, uzi, mp5 - быстро достать ган из инвентаря(ган должен быть на 1 странице инвентаря)
...
changeloglist =[[
{color} Версия v1.1.0
•Добавлена возможность очистить занятую оперативную память(/cclean)
•Добавлена функция автоматической очистки занятой оперативной памяти
•Добавлена функция авто-снятия денег с депозита(подробнее раздел Информация)
•Добавлена функция разделения денег точками
•Переработана система автозаправки, больше не должна ломаться
•Исправлены уведомления о дамаге в телеграмм/vk
•Убраны бесполезные и нерабочие функции
{color} Версия v1.0.9
•Добавлена команда для быстрого звонка игроку по номеру /cnum [номер_телефона]
•Добавлена функция поднятия(/cure) близжайшего игрока
•Переработана система очистки чата от 'ерунды'
•Переработаны функции биндов armour, usedrugs(добавлена возможность выбирать кнопку активации)
•Фикс автозаправки
•Фикс сбора и запуска майнинг ферм
{color} Версия v1.0.8
•Добавлена команда для быстрого звонка игроку по его id /cid [ID]
•Добавлена команда /flock, которая обходит баг "Это не ваш транспорт!"
•Добавлена команда /fnalog для быстрой оплаты налога за семейную квартиру
•Добавлены команды /mp5, /uzi, /ak
•Добавлена функция автоматического нажатия Y/N для квестов
•Изменёно открытие транспорта кнопкой L на основе /flock
•Изменено название "TwinTubo" и "Supreme" на "Hooks Empire"(Будет изменено)
•Фикс функции AutoBreakCar
{color}Версия v1.0.7
•Фикс перевода секунд в минуты мута
•Добавлена команда /getbtc для сбора прибыли с майнинг фермы с помощью "флешки" (подробнее - раздел Информация)
•Добавлена функция использования канистры через чит-код
•Добавлена функция использования рем.комплекта через чит-код
•Добавлен скип диалога удачной покупки в 24/7 / АЗС
•Добавлены сокращенные команды для семьи(/faminvite - /fi, /famuninvite - /fu, fammute - /fm)
•Добавлена функция авто выборы цвета и названия лавки
{color}Версия v1.0.6
...
{color}ПРЕКРАЩЕНИЕ ПОДДЕРЖКИ СКРИПТА ИЗНАЧАЛЬНЫМ РАЗРАБОТЧИКОМ, ВСЕ ОБНОВЛЕНИЯ ВЫШЕ РАЗРАБОТАНЫ S.Hooks
...
local mainIni = inicfg.load({
    config = { -- основные
        ...
        usedrugs_keys = encodeJson({0x33}), -- кнопка usedrugs
        armour_keys = encodeJson({0x32}), -- кнопка armour
        cure_keys = encodeJson({0x34}), -- кнопка cure
        mask_keys = encodeJson({0x31}), -- кнопка mask
        ...
        binds_armour = false, -- бинд /armour
        binds_usedrugs = false, -- бинд /usedrugs
        binds_text_armour = false, -- бинды /do если надел/снял/нет бронежилет(а)
        binds_cure = false, -- бинды /cure
        ...
        autoname = false, -- авто название лавки
        auto_ny = false,
	...
	chest = {
		standart_chest_status = false, -- Открытие сундука стандарт
	    donate_chest_status = false, -- Открытие сундука Донат
	    elonmusk_chest_status = false, -- Открытие сундука Илон Маск
	    platinum_chest_status = false, -- Открытие сундука Платина
	    lossantos_chest_status = false, -- открытие сундука Лос-Сантоса
	    chest_delay = 1800 -- задержка открытия сундуков
	},
...
function main()
	...
	if access_check == false then
		if myname == 'Sam_Hooks' or myname == 'Ron_Hooks' or myname == 'Rick_Hooks' or myname == 'Frank_Hooks' or myname == 'Luni_Lovely' or myname == 'Papa_Hooks' then
			access_check = true
			arzmessage('[Access-Check]: Есть доступ к использованию скрипта')
			arzmessage("Скрипт успешно загружен!\nОткрыть меню - /arz!\nВерсия "..thisScript().version, 5)
		else
			arzmessage('[Access-Check]: Нет доступа к использованию скрипта')
			thisScript():unload()
		end
	end
	...
	sampRegisterChatCommand('getbtc', function(arg)
		if arg:len() > 0 then
			...
			if hnum < -1 then
				sampAddChatMessage('[ARZ Assistant] {ffffff}Введите /getbtc [номер_дома]', 0xFF2F2F)
			end
			if hnum == -1 then
				sampAddChatMessage('[ARZ Assistant] {ffffff}Сбор биткоинов выключен', 0xFF2F2F)
			end
		end
	end)
	sampRegisterChatCommand('startbtc', function(arg)
		if arg:len() > 0 then
			...
			if hnum < -1 then
				sampAddChatMessage('[ARZ Assistant] {ffffff}Введите /startbtc [номер_дома]', 0xFF2F2F)
			end
			if hnum == -1 then
				sampAddChatMessage('[ARZ Assistant] {ffffff}Запуск видеокарт выключен', 0xFF2F2F)
			end
		end
	end)
...
function imgui.RenderMenuItems()
	...
	--[[imgui.SameLine()
	if imgui.Button(fa.ICON_ARCHIVE..u8(' Открытие сундуков'),bsz) then
		ShowListId = 7
	end]]
	imgui.SameLine()
...
function otherelements()
	...
	if imgui.Checkbox(u8'Разделение денег', separator_money) then
		mainIni.config.separator_money = separator_money.v
		saveIniFile()
	end
	imgui.SameLine()
	imgui.TextQuestion(u8'Разделяет денежные суммы точками')
	imgui.SameLine()
	...
	imgui.SameLine()
	imgui.TextQuestion(u8'Надевает бронежилет при нажатии установленной кнопки.')
	...
	imgui.SameLine()
	imgui.TextQuestion(u8'Использует наркотики(3) при нажатии установленной кнопки.')
	...
	if imgui.Checkbox(u8'Укол', binds_cure) then
		mainIni.config.binds_cure = binds_cure.v
		saveIniFile()
	end
	imgui.SameLine()
	imgui.TextQuestion(u8'Поднимает близжайшего игрока при нажатии установленной кнопки.')
	if binds_cure.v then
		imgui.SameLine(111)
	...
	if imgui.Checkbox(u8('Авто название лавки'),autoname) then mainIni.config.autoname = autoname.v saveIniFile() end imgui.SameLine() imgui.TextQuestion(u8('Автоматически вводит название лавки'))
	if imgui.Checkbox(u8('Авто N/Y'),auto_ny) then mainIni.config.auto_ny = auto_ny.v saveIniFile() end imgui.SameLine() imgui.TextQuestion(u8('Автоматическое нажатие клавиш Y и N при тексте на экране'))
	if imgui.Checkbox(u8'Автоочистка памяти', auto_cclean) then
		mainIni.config.auto_cclean = auto_cclean.v
		saveIniFile()
		a_cclean()
	end
	imgui.SameLine()
	imgui.TextQuestion(u8'Автоочистка использованной памяти при достижении выбранного значения')
	if auto_cclean.v then
		imgui.SameLine()
		imgui.PushItemWidth(150)
		if imgui.SliderInt(u8('Очищать при достижении:'),auto_cclean_mem,50,2048) then
...
function autoelements()
	...
	if imgui.Checkbox(u8'Заправлять авто', binds_fillcar) then
		mainIni.config.binds_fillcar = binds_fillcar.v
		saveIniFile()
	end
	imgui.SameLine()
	imgui.TextQuestion(u8'При вводе чит-кода KK ваше авто будет заправлено из канистры.')
	if imgui.Checkbox(u8'Чинить авто', binds_repcar) then
		mainIni.config.binds_repcar = binds_repcar.v
		saveIniFile()
	end
	imgui.SameLine()
	imgui.TextQuestion(u8'При вводе чит-кода RR ваше авто будет починено рем. комплектом.')		
...
function skipdialogselements()
	...
	if state_skips.v then
		...
		if imgui.Checkbox(u8'Скипать диалог покупки в 24/7 / АЗС',succesfulbuy) then
			mainIni.skipdialogs.succesfulbuy = succesfulbuy.v
			saveIniFile()
		end
		if imgui.Checkbox(u8'Скипать диалог с подтверждением трейда',trade_warn) then
	...
	if if_damage.v then
		if damage > 0 then
			if playerId > -1 and playerId < 1001 then
				...
				sendnotification('[WARNING] [damage] Игрок '..nickname..'['..playerId..']'..' damage: '..math.floor(damage)..' weapon: '..weapon..' bodypart: '..bodypart,-1)
			end
		end
	end
	local killer
...
function sampev.onShowDialog(dialogId, dialogStyle, dialogTitle, okButtonText, cancelButtonText, dialogText)
	...
	if state_skips.v then
		...
		if trade_warn.v then
			if dialogId == 0 and dialogText:find('Перед тем как подтвердить сделку, советуем всё тщательно перепроверить.') then
				sampSendDialogResponse(id,0,0,'')
				return false
			end
		end
	...
	if dialogId == 7238 and hnum >= 0 then
		...
		if dialogId == 25181 and hnum >= 0 then
			if num ~= 29 then
				...
				lua_thread.create(function()
					sampAddChatMessage('[ARZ Assistant]{ffffff} Биткоины собраны. Используйте /getbtc [номер_дома] для сбора с другого дома', 0xFF2F2F)
					num = 1
					flashtry = 1
					hnum = -1
					wait(250)
		...
		if dialogId == 25243 and hnum >= 0 then
			if num == 29 then
				...
				if start_btc == 1 then
					hnum = -1
					num = 1				
					sampAddChatMessage('[ARZ Assistant]{ffffff} Видеокарты запущены. Используйте /startbtc [номер_дома] для запуска в другой дом', 0xFF2F2F)
					start_btc = 0
				end
				lua_thread.create(function()
					wait(1000)
			...
			if debug_mining == false and num ~= 29 then
				...
				if start_btc == 1 and dialogText:find('Запустить') then
					lua_thread.create(function()
						wait(250)
				...
				if start_btc == 1 and dialogText:find('Остановить') then
					lua_thread.create(function()
						wait(250)
						sampSendDialogResponse(25243, 0, -1, -1)
						num = num + 1
	...
	if dialogId == 15247 then
		if fnalog == 1 then
			if dialogText:find('$') then
				lua_thread.create(function()
					local c = dialogText:match('Сейчас налог на квартиру составляет {......%}$(%d+).')
					fnalog = 0
					wait(250)
	...
	if dialogId == 235 and deposit_check == 1 then
		if dialogText:find('Деньги на депозите:') then
			lua_thread.create(function()
				local c = dialogText:match('Деньги на депозите: %{......%}%[$(%d+)%]')
...
function sampev.onServerMessage(color, text)
	...
	if call_id == 1 then
		...
		elseif color == 1687547391 and text:find("Номера телефонов государственных служб:") then
		    return false
		elseif text:find("{FFFFFF}1.{6495ED} 111 %- {FFFFFF}Проверить баланс телефона") then
		    return false
		elseif text:find("{FFFFFF}2.{6495ED} 060 %- {FFFFFF}Служба точного времени") then
		    return false
		elseif text:find("{FFFFFF}3.{6495ED} 911 %- {FFFFFF}Полицейский участок") then
		    return false
		elseif text:find("{FFFFFF}4.{6495ED} 912 %- {FFFFFF}Скорая помощь") then
		    return false
		elseif text:find("{FFFFFF}5.{6495ED} 913 %- {FFFFFF}Такси") then
		    return false
		elseif text:find("{FFFFFF}6.{6495ED} 914 %- {FFFFFF}Механик") then
		    return false
		elseif text:find("{FFFFFF}7.{6495ED} 8828 %- {FFFFFF}Справочная центрального банка") then
		    return false
		elseif text:find("{FFFFFF}8.{6495ED} 997 %- {FFFFFF}Служба по вопросам жилой недвижимости") then
		    return false
		end
	end
	if text:find ('Вы заглушены. Оставшееся время %d+ заглушки') then
		if fromSecToMinInMute.v then
			if text:find("Вы заглушены. Оставшееся время заглушки %d+ секунд%.") then
				ct, _ = math.modf(tonumber(text:match("Вы заглушены. Оставшееся время заглушки (%d+) секунд%.")) / 60)
				if ct < 60 then 
					return {color, string.format("Вы заглушены. Оставшееся время заглушки %s секунд = %s минут", text:match("Вы заглушены. Оставшееся время заглушки (%d+)"), ct)}
				else
				ht, _ = math.modf(tonumber(text:match("Вы заглушены. Оставшееся время заглушки (%d+)")) / 60/60)
				return {color, string.format("Вы заглушены. Оставшееся время заглушки %s секунд = %s минут = %s часов", text:match("Вы заглушены. Оставшееся время заглушки (%d+)"), ct, ht)}
				end
			end
		end
	end
	if text:find ('Выводить прибыль можно только') then
		if num ~= 29 then
			lua_thread.create(function()
				flashtry = flashtry + 1
				num = num + 1
	...
	if text:find ('BTC{ffff00}, осталось на счету видеокарты: {ffffff}') and hnum ~= -1 then
		flashtry = flashtry + 1
		num = num + 1
		lua_thread.create(function()
			sendcloseinventory()
	...
	if floodclear.v then  
		...
		if text:find("- Основные команды сервера: /menu /help /gps /settings") and not text:find('говорит') then
			return false
		end
		if text:find("- Пригласи друга и получи бонус в размере $300 000!") and not text:find('говорит') then
			return false
		end
		if text:find("- Наш сайт: arizona-rp.com") and not text:find('говорит') and not text:find('говорит') then
			return false
		end
		...
		if text:find("- Игроки со статусом") and not text:find('говорит') then
			return false
		end
		if text:find("можно приобрести редкие") and not text:find('говорит') then
			return false
		end
		if text:find("предметы, которые выделят тебя из толпы! Наш сайт:") and not text:find('говорит') then
			return false
		end
		if text:find("- Наш сайт:") and not text:find('говорит') then
			return false
		end
		if text:find("[Подсказка] С помощью телефона можно заказать такси. Среднее время ожидания - 2 минуты!") and not text:find('говорит') then
			return false
		end
		...
		if text:find("Чтобы завести двигатель введите") and not text:find('говорит') then
			return false
		end
		if text:find("Чтобы включить радио используйте кнопку") and not text:find('говорит') then
			return false
		end
		if text:find("Для управления поворотниками используйте клавиши:") and not text:find('говорит') then
			return false
		end
		if text:find("В транспорте может присутствует радио") and not text:find('говорит') then
			return false
		end
		if text:find("С помощью телефона можно заказать такси") and not text:find('говорит') then
			return false
		end
		if text:find("Состояние вашего авто крайне плохое! Машина может сломаться!") and not text:find('говорит') then
			return false
		end
		if text:find("Необходимо заехать на станцию тех. обслуживания!") and not text:find('говорит') then
			return false
		end
		if text:find("") and not text:find('говорит') then
			return false
	...
	if text:find('^%s*.+ купил у вас .+, вы получили .+ от продажи') then
		local nick, item, price = text:match('^%s*(.+) купил у вас (.+), вы получили (.+) от продажи')
	...
	if text:find('^%s*Вы купили .+ у игрока .+ за .+') then
		local item, nick, price = text:match('^%s*Вы купили (.+) у игрока (.+) за (.+)')
		if #SHOP_ITEM >= 10 then
			SHOP_ITEM = {}
		end
		sampAddChatMessage(nick..item..price,-1)
	...
	function(id, status, p1, p2)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD and id == int_json_download then
			if doesFileExist(json) then
				...
				self.data.result = false
				self.data.status = '[exist] Ошибка обновления'
				if autoupd then
					arzmessage(self.data.status)
				end
				return false
...
function CheckMemory()
    ...
    sampAddChatMessage(('{6A5ACD}[minus memory]{FFFFFF} Память заполнена на %s МБ.'):format(info), 0x01A0E9)
end
function CClean()
	...
	sampAddChatMessage(("[ARZ Assistant] {FFFFFF}Памяти до: {067d46}%sMB{FFFFFF}. Памяти после: {067d46}%sMB{FFFFFF}. Очищено: {067d46}%sMB{FFFFFF}."):format(under, after, cleaned), 0x01A0E9)
end
-- new function
function theme1()
	local style = imgui.GetStyle()
